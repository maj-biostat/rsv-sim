...
|Outcome: |  Any occurrence of medically attended RSV-LRI (RSV-LRI) in the 12 months after randomisation |   
...
| 0.3 | 2025-09-10 | Update accrual process, vary enrol rate, refactor ref design |
|     |            | Added example data for outcomes, example trials |
...
Included are the data generation assumptions, modelling, simulation scenarios and the results.
...
Participants enter the study when the infant is 6 months (+90 days^[Possibly excessively large window that would necessitate some deeper consideration for age in our modeling.]) and are randomised (1:1) to standard of care versus a single dose of nirsevimab (active).
...
Region is based on a multinomial distribution with two levels with the probability of residing in Alice^[Used as the reference level in the linear predictor.] set to 0.6.
...
Occurrence of medically attended RSV-LRI in the `r l[[1]]$cfg$fu_lab` after randomisation is simulated as a bernoulli random variable for each participant with probability computed from a linear risk model.
...
If no decisions are made in the interim analyses, the final analysis occurs when the maximum sample size is reached and all participants have reached their primary endpoint.

If the treatment strategy is successful, then the probability of medically attended RSV-LRI in the `r l[[1]]$cfg$fu_lab` after randomisation will be lower in the treatment arm than it will under standard of care, which would lead to negative values for the risk difference for the treatment effect.
...
+ Futility: low probability that the risk difference is below some small negative reference value.
...
The scenarios are purposely not detailed so that the results can be discussed without the cognitive bias associated with expected results.
...
for(i in 1:length(l)){
  ...
  d_tmp <- d_all[, .(tfu_max = max(tfu/365)), by = sim]
  # average over sims
  d_fu_ref <- rbind(
    d_fu_ref, 
    data.table(
      id = i, desc = l0_cfg$desc, 
      fu_mu_ref = mean(d_tmp$tfu_max), fu_sd_ref = sd(d_tmp$tfu_max)
    ))
  
  # for the adaptive trial, we would stop at the analysis at which the 
  # decision was triggered so we consider ia (the analysis sets) rather than
  # ic (the enrolment cohort)
  d_all <- copy(l[[i]]$d_all)
  d_tmp <- d_all[!is.na(ia), .(tfu_max = max(tfu/365, na.rm = T)), by = sim]
  
  # pick  out the last interim by sim and then compute the average across the
  # region and locality strata
  # d_last <- d_all[
  #   d_all[, .I[ic == max(ic)], by = sim]$V1, .(tfu = mean(tfu/365)), keyby = sim]
  ...
  d_fu <- rbind(
    ...
    data.table(
      ...
      fu_mu = mean(d_tmp$tfu), fu_sd = sd(d_tmp$tfu)
    ))
...
For the fixed design, the final enrolment is expected to reach the `r l[[1]]$cfg$fu_lab` followup at `r sprintf("%.1f", d_fu_ref[, mean(fu_mu_ref)])` years.
For the adaptive trial, the `r l[[1]]$cfg$fu_lab` followup for the final enrolment varies by scenario as shown in @tbl-trial-duration.
In the adaptive trial, the times at which the study can stop are determined based on latest follow up time for the participants that enter the analyses.

```{r, eval = T}
#| echo: FALSE
#| label: tbl-trial-duration
#| tbl-pos: H
#| tbl-cap: "Duration of trial in years (mean and sd)"
...
gt_tbl <- gt(d_tbl[, .SD, .SDcols = !c("id")], 
  ...
  cols_merge(
    columns = c("fu_mu_ref", "fu_sd_ref"),
    pattern = "<<{1}>><< ({2})>>"
  ) |>
  fmt_number(
    columns = c("fu_mu", "fu_sd", "fu_mu_ref", "fu_sd_ref"),
  ...
  tab_spanner(
    label = md("Years for final enrolment to reach followup"),
    columns = c("fu_mu", "fu_sd", "fu_mu_ref", "fu_sd_ref")
    )|>
  cols_align(
    columns = c("fu_mu"),
...
The duration of time to the end of follow up reflects the period of time to follow up the enrolled cohort, which is longer than the duration of follow up for the set of participants that actually enter into the analyses (the analysis set).
...
for(i in 1:length(l)){
  ...
  d_n_trt <- d_all[, .(N = .N), keyby = .(sim, ic, trt)]
...
for(i in 1:length(l)){
  ...
  d_pars <- dcast(d_pars, sim + ic ~ par, value.var = c("mu", "med", "se"))
  
  # plot(density(d_pars[!is.na(med_rd_2_1) & ic == 2, med_rd_2_1]))
  # lines(density(d_pars[!is.na(mu_rd_2_1) & ic == 2, mu_rd_2_1]))
  # 
  # abline(v = (d_pars[!is.na(med_rd_2_1) & ic == 2, mean(med_rd_2_1)]))
  # abline(v = rd_trt, col = 2)
  # 
  # d_pars[!is.na(mu_rd_2_1), .N, keyby = ic]
  # d_pars[, mean(mu_rd_2_1, na.rm = T), keyby = ic]
  # d_pars[!is.na(mu_rd_2_1), mean(mu_rd_2_1), keyby = ic]
  # d_pars[!is.na(mu_rd_2_1) & ic == 3, ]
  
  # plot(density(d_pars[ic == 1, mu_rd_2_1]))
  # plot(density(d_pars[ic == 2 & !is.na(mu_rd_2_1), mu_rd_2_1]))
...
for(i in 1:length(l)){
  ...
  d_all <- d_all[, .(y = sum(y), N = .N), keyby = .(trt, reg, loc)]
  d_all[, p_obs := y / N]
