...
d_tbl <- data.table(
  ...
  spec = c(
    "All First Nations infants age 6 months at enrolment, including infants who received passive antibody protection from maternal RSV vaccination, infants who received first dose nirsevimab at birth, infants who received both maternal RSV vaccine protection and nirsevimab, and infants who received neither.",
    "Intervention group: Single IM dose of 50mg in 0.5mL nirsevimab (100mg for infants > 5kg). Control: standard care.  Randomisation occurs on enrolment (at 6 months) and dose administration (or none) occurs immediately. Open label.",
    "Medically attended RSV-LRI: ascertained via review of notifications for laboratory-confirmed RSV infection to the NT Notifiable Disease System from time of randomisation to 6-months or 12-months (TBD) post randomisation date.",
    "Risk difference between treatment groups",
    "Treatment policy. ICEs that impact the interpretation of the outcome: non-receipt of treatment (e.g. treatment cannot take place), delayed treatment, wrong dose, use of additional RSV prophylaxis. ICEs that impact the existence of the outcome: death, access to clinics prevents testing, inter-state migration leading to use of other registry, use of medicines that mask symptoms and lead to not testing, other conditions that have similar symptoms and lead to not testing. Potential for differential dropout due to open label (control group has less to gain from involvement)."
  )
)

gt(d_tbl) |>
  cols_label(
    attr = "Estimand Component",
    spec = ""
  ) |>
  tab_options(
...
- analyses after 600, 800, 1000 have reached 12-month FU
- 1:1 rand within baseline covariate grouping (region, locality)
...
- todo: stratified rand on baseline immunisation state
- todo: example trials
- todo: consider impact of RSV before 6 months

## Probability model

- Logistic regresssion (weakly informative priors)
- Parameters (log-OR) transformed to risk differences
...
- If niservimab is beneficial, probability of MA-RSV LRI will be lower in treatment group.

. . .

- Superiority - more than 0.975 of the posterior density for the RD(trt) below 0
...
d_tbl_1_cur <- dcast(
  d_tbl_1_cur, 
  scenario + desc + rd_trt + decision + pct + ty ~ . , 
  value.var = list("N_mu"))
...
+ AD type-i assertion probability $\le$ 5%
+ AD >80% power at 4.5% risk difference when assuming a baseline risk around 10%
+ AD generally shorter in duration than fixed design

. . .

+ AD will result in inconclusive result around 30% of time under null scenario
+ AD more likely to conclude futility (lack of efficacy) than fixed design under all scenarios
